...
// l'état accepté.
export function gabaritDevis({ config, numero, clientNom, total, lien, joursValidite = 30, dejaAccepte = false, relance = false, pdfJoint = false }) {
  return enveloppe(
    config,
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour${clientNom ? ` ${clientNom}` : ""},</p>
     <p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">
       ${relance
         ? `Petit rappel : votre devis <strong>${numero}</strong>${total ? ` au montant de <strong>${total}</strong> (avant taxes)` : ""} attend toujours votre réponse. Vous pouvez le consulter et nous répondre en ligne — un clic suffit :`
         : dejaAccepte
     ...
     ${pdfJoint ? `<p style="margin:0 0 12px;color:#334155;font-size:13px;line-height:1.5;">📎 Le devis est aussi joint à ce courriel <strong>en PDF</strong> — pour l'imprimer ou le conserver.</p>` : ""}
...
export function gabaritFactureMaison({ config, numero, clientNom, total, lien, echeance = null, credit = false }) {
  return enveloppe(
    config,
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour${clientNom ? ` ${clientNom}` : ""},</p>
     <p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">
     ...
     <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto 16px;"><tr><td style="background:#131B2E;border-radius:8px;">
       <a href="${lien}" style="display:inline-block;padding:12px 28px;color:#ffffff;font-size:14px;font-weight:bold;text-decoration:none;">${credit ? "Voir ma note de crédit" : "Voir ma facture"}</a>
     </td></tr></table>
     <p style="margin:0;color:#64748b;font-size:12px;line-height:1.5;">
...
export function gabaritConfirmationRdv({ config, clientNom, date, adresse, technicien, depotRecu = true }) {
  return enveloppe(
    config,
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour${clientNom ? ` ${clientNom}` : ""},</p>
     <p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">
       ${depotRecu ? "Nous avons bien reçu votre dépôt — merci ! " : ""}Votre rendez-vous est confirmé :
     </p>
     <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 16px;background:#f8fafc;border-radius:8px;width:100%;">
       <tr><td style="padding:14px 18px;color:#0f172a;font-size:14px;line-height:1.8;">
         ...
         ${adresse ? `📍 ${adresse}<br/>` : ""}
         ${technicien ? `👷 Votre technicien : <strong>${technicien}</strong><br/>` : ""}
         🕐 Nous vous contacterons la veille pour confirmer le rendez-vous.
       </td></tr>
     </table>
     <p style="margin:0;color:#64748b;font-size:12px;line-height:1.5;">
       Un empêchement ? Répondez à ce courriel ou appelez-nous${config?.telephone ? ` au ${config.telephone}` : ""} — nous trouverons un autre moment.
     </p>`
  );
}
...
export function gabaritEnRoute({ config, clientNom, technicien, delaiMinutes, adresse }) {
  const delai = Number(delaiMinutes) > 0 ? `environ ${Number(delaiMinutes)} minutes` : "bientôt";
  return enveloppe(
    config,
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour${clientNom ? ` ${clientNom}` : ""},</p>
     <p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">
       ${technicien ? `<strong>${technicien}</strong>, votre technicien, est en route` : "Votre technicien est en route"} — arrivée prévue dans <strong>${delai}</strong>.
     </p>
     ${adresse ? `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 16px;background:#f8fafc;border-radius:8px;width:100%;">
       <tr><td style="padding:12px 18px;color:#0f172a;font-size:14px;line-height:1.7;">📍 ${adresse}</td></tr>
     </table>` : ""}
     <p style="margin:0;color:#64748b;font-size:12px;line-height:1.5;">
       Un empêchement de dernière minute ? Appelez-nous${config?.telephone ? ` au ${config.telephone}` : ""}.
     </p>`
  );
}
...
export function gabaritBonTravail({ config, clientNom, lien, joursValidite = 90, lienAvis = null }) {
  return enveloppe(
    config,
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour${clientNom ? ` ${clientNom}` : ""},</p>
     <p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">
     ...
     ${lienAvis ? `<p style="margin:0 0 12px;border-top:1px solid #e2e8f0;padding-top:14px;color:#334155;font-size:13px;line-height:1.6;">
       ...
       <a href="${lienAvis}" style="color:#2563eb;font-weight:bold;">⭐ Laisser un avis Google</a>
     </p>` : ""}
     <p style="margin:0;color:#64748b;font-size:12px;line-height:1.5;">
       Si le bouton ne fonctionne pas, copiez cette adresse dans votre navigateur :<br/>
       <a href="${lien}" style="color:#2563eb;word-break:break-all;">${lien}</a>
...
export function conditionsDepotAppel(config, lienConditions = null) {
  const nom = config?.nomLegal || config?.nomCommercial || "notre entreprise";
  return (
...
export function gabaritDemandePaiement({ config, clientNom, description, lignes, tps, tvq, total, lienPaiement = null, conditions = null, termesHtml = null, lienConditions = null }) {
  ...
  return enveloppe(
    ...
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour${clientNom ? ` ${clientNom}` : ""},</p>
     ...
     ${conditions ? `<p style="margin:0 0 16px;padding:12px 14px;background:#fffbeb;border:2px solid #f59e0b;border-radius:8px;color:#78350f;font-size:13px;line-height:1.6;">
       <span style="display:block;font-weight:bold;font-size:13px;margin-bottom:4px;">⚠️ À lire avant de payer</span>
       ${conditions}
     </p>` : ""}
     ...
     ${conditions ? `<p style="margin:0 0 16px;text-align:center;color:#64748b;font-size:12px;line-height:1.5;">
       En payant, vous confirmez avoir pris connaissance des conditions ci-dessus${lienConditions ? ` —
       <a href="${lienConditions}" style="color:#2563eb;font-weight:bold;">conditions complètes</a>` : ""}${termesHtml ? `, reproduites intégralement au bas de ce courriel` : ""}.
     </p>` : ""}
...
function blocFichiersBc(fichiers) {
  ...
  return (
    ...
    liste.map((f) => `<li><a href="${f.url}" style="color:#0f172a;">📎 ${String(f.nom || "fichier").replace(/</g, "&lt;")}</a></li>`).join("") +
    `</ul>`
  );
}
...
export function gabaritBcSimple({ config, numeroBc, description, adresseLivraison, photos = [], fichiers = [] }) {
  return enveloppe(
    config,
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour,</p>
     <p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">
       Veuillez traiter le bon de commande <strong>${numeroBc || "(numéro à venir)"}</strong> :
     </p>
     <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin:0 0 16px;">
       <tr><td style="padding:6px 8px;border:1px solid #e2e8f0;background:#f8fafc;font-size:13px;font-weight:bold;color:#0f172a;">Commande</td>
           <td style="padding:6px 8px;border:1px solid #e2e8f0;font-size:13px;color:#334155;white-space:pre-line;">${description || "(voir avec nous)"}</td></tr>
     </table>
     ${adresseLivraison ? `<p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">
       Livraison : <strong>${adresseLivraison}</strong>.
     </p>` : ""}
