...
export function conditionsDepotAppel(config) {
  ...
  return (
    ...
    `générales de ${nom}, disponibles sur demande.`
  );
}

export function gabaritDemandePaiement({ config, clientNom, description, lignes, tps, tvq, total, lienPaiement = null, conditions = null }) {
  ...
  return enveloppe(
    ...
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour${clientNom ? ` ${clientNom}` : ""},</p>
     ...
     ${conditions ? `<p style="margin:12px 0 0;padding:10px 12px;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;color:#78350f;font-size:12px;line-height:1.55;">⚠️ ${conditions}</p>` : ""}`
  );
}
