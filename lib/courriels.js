...
export async function envoyerCourriel({ a, sujet, html, copieExpediteur = false, copieA = [] }) {
  try {
    ...
    const reponse = await fetch("/api/courriel", {
      ...
      body: JSON.stringify({ a, sujet, html, copieExpediteur, copieA }),
    });
...
// l'état accepté.
export function gabaritDevis({ config, numero, clientNom, total, lien, joursValidite = 30, dejaAccepte = false, relance = false }) {
  return enveloppe(
    config,
...
export function gabaritBcSimple({ config, numeroBc, description, adresseLivraison, photos = [] }) {
  return enveloppe(
    config,
...
export function gabaritBonCommande({ config, piece, photos = [] }) {
