...
export async function envoyerCourriel({ a, sujet, html }) {
  try {
    ...
    const reponse = await fetch("/api/courriel", {
      ...
      body: JSON.stringify({ a, sujet, html }),
    });
...
export function gabaritBonCommande({ config, piece }) {
  ...
  return enveloppe(
    ...
    `<p style="margin:0 0 12px;color:#0f172a;font-size:15px;">Bonjour,</p>
     ...
     <p style="margin:0;color:#334155;font-size:14px;line-height:1.5;">
       Merci de confirmer la réception de cette commande et, si possible, la <strong>date de livraison prévue</strong>.
     </p>`
  );
}
