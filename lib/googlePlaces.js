...
export async function detailsAdresse(suggestion, jeton) {
  ...
  await place.fetchFields({
    // `location` (2026-09-09) : les coordonnées GPS de l'adresse — pour
    // l'estimation du trajet de retour des entreprises clientes.
...
// 📍 GÉOCODER UNE ADRESSE EN TEXTE (2026-09-09) — pour l'estimation du
// trajet de retour des ENTREPRISES CLIENTES : leur adresse (Paramètres)
// est du texte sans coordonnées ; DGL avait les siennes écrites en dur.
// Une seule recherche + un détail (une unité de facturation), résultat
// MIS EN CACHE dans le navigateur par adresse — Google n'est rappelé
// que si l'adresse change. Jamais bloquant : null en cas d'échec.
export async function geocoderAdresse(texte) {
  const propre = String(texte || "").trim();
  if (!propre || !googlePlacesDisponible()) return null;
  const cle = `geo-adresse-${propre.toLowerCase()}`;
  try {
    const enCache = JSON.parse(localStorage.getItem(cle) || "null");
    if (enCache?.lat != null && enCache?.lng != null) return enCache;
  } catch { /* cache illisible — on regéocode */ }
  try {
    const jeton = await nouveauJeton();
    const suggestions = await chercherAdresses(propre, jeton);
    if (!suggestions.length) return null;
    const d = await detailsAdresse(suggestions[0], jeton);
    if (d?.lat == null || d?.lng == null) return null;
    const resultat = { lat: d.lat, lng: d.lng };
    try { localStorage.setItem(cle, JSON.stringify(resultat)); } catch { /* stockage plein — tant pis */ }
    return resultat;
  } catch {
    return null;
  }
