...
// Copie comptable d'une facture dans Sage (phase 3) — appelée APRÈS la
// création de la facture maison : { clientId, clientNom, lignes:
// [{ description, quantite, prixUnitaire }], reference, date }.
export async function creerFactureSage(donnees) {
  try {
    const jeton = await jetonSession();
    if (!jeton) return { erreur: "Session expirée — reconnecte-toi." };
    const reponse = await fetch("/api/sage/facture", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${jeton}` },
