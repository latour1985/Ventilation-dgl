...
// ============================================================
// 🧾 FACTURES DE VENTE (phase 3 du chantier, 2026-09-07) — la facture
// CLIENT reste la facture MAISON de Fluxya (numérotation, page
// publique, courriel) ; Sage reçoit la COPIE COMPTABLE : une
// sales_invoice au nom du contact relié, notre numéro en référence.
//
// ⚠️ Sage exige sur CHAQUE ligne un compte de grand livre
// (ledger_account_id) et un taux de taxe (tax_rate_id). Phase 3 :
// le compte de VENTES par défaut du dossier et un taux à 0 % (le HT
// voyage — les taxes réelles du régime vivent sur la facture maison).
// Le raffinement TPS/TVQ par taux Sage canadien viendra quand un vrai
// dossier Sage CANADA sera branché (l'essai de test est britannique).
// ============================================================

// Le compte de ventes et le taux 0 % du dossier — relus à la demande
// (2 lectures par facture : rare et sans état à rafraîchir).
async function reglagesVenteSage(acces) {
  const comptes = await requeteSage(acces, "ledger_accounts?visible_in=sales&items_per_page=200");
  const listeComptes = comptes?.$items || [];
  const compte =
    listeComptes.find((c) => /sales|ventes/i.test(String(c.displayed_as || c.name || ""))) || listeComptes[0] || null;
  if (!compte?.id) throw new Error("Aucun compte de ventes trouvé dans le dossier Sage.");
  const taux = await requeteSage(acces, "tax_rates?items_per_page=200");
  const listeTaux = taux?.$items || [];
  const zero =
    listeTaux.find((t) => Number(t.percentage) === 0 && /no|zero|exempt/i.test(String(t.displayed_as || t.name || ""))) ||
    listeTaux.find((t) => Number(t.percentage) === 0) ||
    listeTaux[0] ||
    null;
  if (!zero?.id) throw new Error("Aucun taux de taxe trouvé dans le dossier Sage.");
  return { compteId: compte.id, tauxId: zero.id };
}

// Crée la COPIE COMPTABLE d'une facture dans Sage. `lignes` :
// [{ description, quantite, prixUnitaire }] en HT. `reference` = le
// numéro de la facture maison — le fil entre les deux registres.
export async function creerFactureSage(acces, admin, { clientId, clientNom, lignes, reference, date }) {
  let contactId = null;
  if (clientId) {
    contactId = await contactSagePour(acces, admin, { clientId, clientNom });
  } else {
    // Facture sans fiche (rare) : l'homonyme Sage est relié, sinon un
    // contact minimal est créé — jamais de facture orpheline.
    const nom = String(clientNom || "").trim();
    if (!nom) throw new Error("Facture sans client — impossible de créer la copie Sage.");
    const recherche = await requeteSage(acces, `contacts?search=${encodeURIComponent(nom)}&contact_type_id=CUSTOMER&items_per_page=25`);
    const homonyme = (recherche?.$items || []).find((c) => nomNormaliseSage(c?.name || c?.displayed_as) === nomNormaliseSage(nom));
    contactId = homonyme?.id || null;
    if (!contactId) {
      const cree = await requeteSage(acces, "contacts", {
        method: "POST",
        body: JSON.stringify({ contact: { name: nom.slice(0, 200), contact_type_ids: ["CUSTOMER"] } }),
      });
      contactId = cree?.id || null;
    }
    if (!contactId) throw new Error("Sage n'a pas retourné de contact pour la facture.");
  }
  const { compteId, tauxId } = await reglagesVenteSage(acces);
  const facture = await requeteSage(acces, "sales_invoices", {
    method: "POST",
    body: JSON.stringify({
      sales_invoice: {
        contact_id: contactId,
        date: date || new Date().toISOString().slice(0, 10),
        ...(reference ? { reference: String(reference).slice(0, 25) } : {}),
        invoice_lines: (lignes || []).map((l) => ({
          description: String(l.description || "Travaux").slice(0, 200),
          quantity: Number(l.quantite) > 0 ? Number(l.quantite) : 1,
          unit_price: Math.round((Number(l.prixUnitaire) || 0) * 100) / 100,
          ledger_account_id: compteId,
          tax_rate_id: tauxId,
        })),
      },
    }),
  });
  if (!facture?.id) throw new Error("Sage n'a pas retourné d'identifiant de facture.");
  return { factureId: String(facture.id), numero: facture.invoice_number || facture.displayed_as || null };
}
