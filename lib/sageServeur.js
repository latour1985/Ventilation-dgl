...
function chargeContactSage(fiche) {
  ...
  return {
    contact: {
      name: String(fiche.nom || "").trim(),
      contact_type_ids: ["CUSTOMER"],
      ...(defaut?.email ? { email: defaut.email } : {}),
      ...(fiche.telephone ? { telephone: fiche.telephone } : {}),
      // L'adresse de facturation voyage en une ligne — Sage l'accepte
      // et l'affiche telle quelle sur les documents.
      ...(fiche.adresse_facturation
        ? { main_address: { address_line_1: String(fiche.adresse_facturation).slice(0, 200), address_type_id: "SALES", is_main_address: true } }
        : {}),
    },
  };
}
...
export async function contactSagePour(acces, admin, { clientId, clientNom }) {
  const { data: fiche } = await admin
    .from("clients_app")
    .select("id, nom, entreprise, courriels, telephone, adresse_facturation, sage_contact_id")
    .eq("id", clientId)
    .maybeSingle();
