...
export function itemsDepuisCsv(texte) {
  ...
  if (lignes.length === 0) return { items: [], ignorees: 0, mapping: {}, entetes: [], donnees: [] };
  const entetes = lignes[0].map((e) => String(e || "").trim());
  const mapping = repererColonnes(entetes);
  const donnees = lignes.slice(1);
  return { entetes, donnees, mapping, ...itemsDepuisLignes(donnees, mapping) };
}

// Reconstruit les items à partir d'un mapping DONNÉ — c'est ce qui
// permet à l'utilisateur de corriger une colonne mal devinée sans
// recharger son fichier (« les prix sont à zéro » : la colonne de prix
// portait un nom que la reconnaissance automatique ne connaissait pas).
export function itemsDepuisLignes(donnees, mapping) {
  if (!mapping || mapping.nom === undefined) return { items: [], ignorees: (donnees || []).length };
  const items = [];
