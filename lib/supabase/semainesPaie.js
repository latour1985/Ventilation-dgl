...
export async function marquerSemainePayee(debutISO, par) {
  // INSERT simple (pas d'upsert) : la table n'a volontairement AUCUNE
  // policy de mise à jour. Déjà marquée (23505) = déjà fait, pas une erreur.
  const { error } = await supabase.from("semaines_paie").insert({ debut_semaine: debutISO, payee_le: new Date().toISOString(), payee_par: par || null });
