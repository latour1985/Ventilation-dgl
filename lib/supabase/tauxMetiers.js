...
export async function tauxEtSecteurPourCourriel(courriel, secteurTache = "commercial") {
  ...
  const { data: emp } = await supabase
    ...
    .maybeSingle();
  // 🔒 TECHNICIEN : depuis la RLS phase 2, il ne peut PAS lire les
  // salaires — la lecture ci-dessus revient vide et son taux se figeait
  // à NULL (heures à 0 $ dans la rentabilité). Repli (snippet 133) : une
  // fonction serveur SECURITY DEFINER lui rend UNIQUEMENT SON PROPRE
  // taux (le courriel du jeton), jamais celui d'un collègue.
  if (!emp) {
    try {
      const { data } = await supabase.rpc("fn_mon_taux_coutant", { secteur_tache: secteurTache });
      const ligne = Array.isArray(data) ? data[0] : data;
      if (ligne && Number(ligne.taux) > 0) {
        return { taux: Number(ligne.taux), secteurPaie: ligne.secteur_paie || secteurTache };
      }
    } catch {
      // fonction absente (snippet pas encore passé) — taux null, repli
      // d'affichage de l'analyse prend le relais.
    }
    return { taux: null, secteurPaie: secteurTache };
  }
