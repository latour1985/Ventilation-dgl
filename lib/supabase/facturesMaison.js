...
// 🛡️ FINALISATION ATOMIQUE (snippet 141, audit 2026-09-09) : marquer la
// facture « envoyée » ET sortir le bon de la pile « à facturer » dans
// UNE transaction — une coupure entre les deux laissait une facture
// reçue par le client avec un bon encore facturable (double facture au
// deuxième clic). Repli : les deux écritures d'avant, l'une après
// l'autre, si le snippet n'est pas encore passé.
export async function finaliserFactureMaison({ factureId, statut, envoyeeLe, courriels, bonRowId, factures, statutBon }) {
  const { error } = await supabase.rpc("finaliser_facture_maison", {
    p_facture_id: factureId || null,
    p_statut: statut || null,
    p_envoyee_le: envoyeeLe || null,
    p_courriels: courriels || null,
    p_bon_id: bonRowId || null,
    p_factures: factures || null,
    p_statut_bon: statutBon || null,
  });
  if (error) {
    const absente = error.code === "PGRST202" || /find the function|does not exist/i.test(String(error.message || ""));
    if (!absente) throw error;
    if (factureId && statut) await majFactureMaison(factureId, { statut, envoyeeLe: envoyeeLe || undefined, courriels: courriels || undefined });
    if (bonRowId) {
      const { error: e3 } = await supabase
        .from("bons_travail")
        .update({ factures_emises: factures || [], ...(statutBon ? { statut_facturation: statutBon } : {}) })
        .eq("id", bonRowId);
      if (e3) throw e3;
    }
  }
...
// 👁️ « Le client a consulté sa facture » (snippet 123) — page publique,
// fire-and-forget, jamais en mode aperçu (bureau).
export function noterConsultationFactureMaison(jeton) {
  if (!jeton) return;
  supabase.rpc("noter_consultation_facture_maison", { p_jeton: jeton }).then(
    () => {},
    () => {}
  );
}
