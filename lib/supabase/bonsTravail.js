...
// Le MOULE de la ligne bons_travail — partagé entre l'écriture directe
// (ecrireBon) et la fermeture atomique bon+heures (snippet 141), pour
// que les deux chemins produisent EXACTEMENT la même ligne.
export function construireLigneBon(infos, email, nom) {
  return {
      ...
      // ✅ Étapes de la job cochées sur le terrain (2026-09-09, snippet 139).
      etapes: Array.isArray(infos.etapes) && infos.etapes.length > 0 ? infos.etapes : null,
...
async function ecrireBon(infos, email, nom) {
  const ligne = construireLigneBon(infos, email, nom);
  const ecrire = (charge) =>
    supabase
      .from("bons_travail")
  ...
  // au bout.
  if (error && (error.code === "PGRST204" || /travaux_non_termines|reste_a_faire|\betapes\b/.test(error.message || ""))) {
    const { travaux_non_termines: _a, reste_a_faire: _b, etapes: _c, ...sansNouvelles } = ligne;
...
// 👥 L'APPORT DES COÉQUIPIERS (2026-08-27) : photos et notes déjà
// enregistrées par les AUTRES techniciens de la même tâche (clé exacte
// + clés « id::jour » des chantiers multi-jours). Le DERNIER qui ferme
// s'en sert pour que le bon envoyé au client porte le travail de TOUTE
// l'équipe — pas seulement le sien (constat des techniciens : « nous ne
// devons pas perdre l'information si un technicien finit en premier »).
// Best effort : hors-ligne ou table injoignable → vide, le bon part
// comme avant, jamais bloquant.
export async function apportEquipePourBon(tacheId, monCourriel) {
  const vide = { photosAvant: [], photosApres: [], notes: [] };
  if (!tacheId) return vide;
  try {
    const { data, error } = await supabase
      .from("travaux_effectues")
      .select("employe_email, employe_nom, note_terrain, photos")
    ...
    const notes = [];
    (data || [])
      .filter((r) => (r.employe_email || "").toLowerCase() !== moi)
      .forEach((r) => {
        (Array.isArray(r.photos?.avant) ? r.photos.avant : []).forEach((u) => photosAvant.push(u));
        (Array.isArray(r.photos?.apres) ? r.photos.apres : []).forEach((u) => photosApres.push(u));
        if ((r.note_terrain || "").trim()) {
          notes.push({ nom: r.employe_nom || r.employe_email || "coéquipier", note: r.note_terrain.trim() });
        }
      });
    return { photosAvant: [...new Set(photosAvant)], photosApres: [...new Set(photosApres)], notes };
  } catch {
    return vide;
  }
}
...
export async function listerBonsTravail() {
  // 📚 Par pages (bilan 2026-09-03) : l'ancien plafond de 300 aurait fait
  // disparaître de vieux bons non facturés de la pile, en silence.
  const data = await lireParPages(() =>
    supabase.from("bons_travail").select("*").order("envoye_le", { ascending: false }).order("id")
  );
  return (data || []).map((row) => ({
    id: `sbb-${row.id}`,
    supabase: true,
    // 🔐 L'entreprise du bon — un compte plateforme (clé principale) lit
    // les bons de TOUTES les entreprises ; l'app doit pouvoir écarter
    // ceux qui ne sont pas de la compagnie affichée (2026-09-17).
    ...
    // 👁️ Consultations du client (snippet 123).
    consulteLe: row.consulte_le || null,
    consultations: Number(row.consultations) || 0,
    derniereConsultationLe: row.derniere_consultation_le || null,
    ...
    montant: 0,
    prixNonListe: true,
    // ✍️ RÉVISION ENREGISTRÉE (snippet 150, 2026-09-18) : une révision
    // validée survit au rechargement — ses lignes et son total
    // remplacent le « à réviser » par défaut. Absente = comme avant.
    ...(Array.isArray(row.revision?.lignes) && row.revision.lignes.length > 0
      ? {
          montant: Number(row.revision.montant) || 0,
          lignesNonListees: row.revision.lignes,
          prixNonListe: false,
          revisionValideeLe: row.revision.valideLe || null,
          revisionValideePar: row.revision.validePar || null,
        }
      : {}),
    statutQb:
...
// ✍️ RÉVISION D'UN BON (snippet 150, 2026-09-18) — la révision validée
// s'enregistre SUR le bon : elle survit au rechargement et peut être
// rouverte avant l'envoi. `revision` = { lignes, montant, valideLe,
// validePar }. Retourne false si la colonne n'existe pas encore
// (snippet pas passé) — l'appelant le dit au journal, rien ne casse.
export async function sauvegarderRevisionBon(rowId, revision) {
  const { data, error } = await supabase.from("bons_travail").update({ revision: revision || null }).eq("id", rowId).select("id");
  if (error && (error.code === "PGRST204" || /revision/.test(error.message || ""))) return false;
  if (error) throw error;
  // 0 ligne touchée = refusé en silence par la sécurité (revue 2026-09-22) :
  // on ne dit plus « enregistrée » à tort.
  if ((data || []).length === 0) throw new Error("La révision n'a pas pu être enregistrée (bon introuvable ou accès refusé).");
  return true;
}

// La fonction Postgres du snippet n'est pas encore passée ? On le
// reconnaît pour retomber sur l'ancien chemin plutôt que d'échouer.
export function fonctionSqlAbsente(e) {
  return e?.code === "PGRST202" || /find the function|does not exist/i.test(String(e?.message || ""));
}

// 🛡️ MARQUAGE EN LOT — TOUT OU RIEN (snippet 141, audit 2026-09-09).
// La facture groupée marquait ses N bons un par un : une coupure au
// milieu laissait des bons « à facturer » DÉJÀ facturés → double
// facturation au prochain passage. Ici les N marquages entrent dans
// UNE transaction Postgres. `lots` = [{ rowId, factures, statut }].
// Repli : si le snippet n'est pas passé, l'ancien chemin un-par-un.
export async function poserFacturesEmisesLot(lots) {
  const charge = (lots || []).map((l) => ({ id: l.rowId, factures: l.factures || [], statut: l.statut || null }));
  if (charge.length === 0) return 0;
  const { data, error } = await supabase.rpc("poser_factures_emises_lot", { p_lots: charge });
  if (error) {
    if (!fonctionSqlAbsente(error)) throw error;
    for (const l of lots) await majFacturesEmises(l.rowId, l.factures, l.statut);
    return lots.length;
  }
  return Number(data) || charge.length;
}
...
export const RAISONS_RETRAIT = {
  ...
  // 🧾 Déjà facturé AILLEURS (2026-09-09, demande №6 du propriétaire) :
  // la job a été facturée hors Fluxya (ancien système, entente
  // spéciale) — le bon quitte la pile sans jamais refacturer.
  facture_hors_fluxya: "Déjà facturé hors Fluxya — ne pas refacturer ici",
};
