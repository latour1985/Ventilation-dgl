...
function versUi(row) {
  return {
    ...
    // ⚠️ Réception partielle (snippet 155).
    ...
    // 🚚 Ramassage chez le fournisseur (snippet 154) — absents avant le snippet.
    ramassage: !!row.ramassage,
...
export async function majPiece(id, champs) {
  let { error } = await supabase.from("pieces_commandees").update(champs).eq("id", id);
  // Filet snippet 154 pas passé : la commande s'enregistre sans les
  // champs de ramassage plutôt que d'échouer en bloc.
  if (error && /ramassage|ramasse_par|depot_a/.test(error.message || "")) {
    const { ramassage: _r, ramasse_par: _p, depot_a: _d, ...sans } = champs;
    if (Object.keys(sans).length === 0) return;
    ({ error } = await supabase.from("pieces_commandees").update(sans).eq("id", id));
  }
  if (error) throw error;
}

// ⚠️ RÉCEPTION PARTIELLE (snippet 155, 2026-09-22) — la pièce reste
// « commandée » (la tâche reste bloquée) ; on note ce qui manque, la
// date promise pour le reste, la réclamation. Message clair si le
// snippet n'est pas passé.
export async function majReceptionPartiellePiece(id, { manquant, restePromisLe, partielLe, reclameLe } = {}) {
  const champs = {};
  if (manquant !== undefined) champs.manquant = manquant || null;
  if (restePromisLe !== undefined) champs.reste_promis_le = restePromisLe || null;
  if (partielLe !== undefined) champs.partiel_le = partielLe || null;
  if (reclameLe !== undefined) champs.reclame_le = reclameLe || null;
  if (Object.keys(champs).length === 0) return;
  const { error } = await supabase.from("pieces_commandees").update(champs).eq("id", id);
  if (error) {
    if (/manquant|reste_promis_le|partiel_le|reclame_le/.test(error.message || "")) throw new Error("Passe le SQL « 155 - Réception partielle » pour activer cette fonction.");
    throw error;
  }
}
