...
function versUi(row) {
  return {
    ...
    // 📎 Dépliant du produit (snippet 149) : joint AUTOMATIQUEMENT au
    // devis dès que cet item y figure. Un seul fichier par item.
    depliantUrl: row.depliant_url || null,
    depliantNom: row.depliant_nom || "",
  };
}

export async function listerCatalogue() {
  // 📚 Par pages (bilan 2026-09-03) : limit(2000) mentait — Supabase
  // plafonne à 1 000 quoi qu'on demande.
  const data = await lireParPages(() =>
    supabase.from("catalogue_items").select("*").eq("actif", true).order("nom").order("id")
  );
...
export async function sauvegarderItem(item) {
  ...
  // 📎 Dépliant (snippet 149) — écrit seulement s'il est fourni, pour ne
  // pas effacer un dépliant existant lors d'une sauvegarde partielle.
  if (item.depliantUrl !== undefined) ligne.depliant_url = item.depliantUrl || null;
  if (item.depliantNom !== undefined) ligne.depliant_nom = item.depliantNom || null;
...
export async function enregistrerItemsEnLot(items) {
  ...
  // Noms refusés par une contrainte d'unicité — rapportés à l'appelant
  // plutôt que de faire échouer TOUT l'import (2026-08-28).
  const ignores = [];
  for (let debut = 0; debut < lignes.length; debut += TRANCHE) {
    const tranche = lignes.slice(debut, debut + TRANCHE);
    const { data, error } = await supabase.from("catalogue_items").upsert(tranche).select();
    if (!error) {
      sauves.push(...(data || []).map(versUi));
      continue;
    }
    // ⚠️ UN SEUL nom en double faisait tomber la tranche ENTIÈRE (« rien
    // n'a été appliqué ») : 99 items corrects perdus à cause d'un.
    // On rejoue alors la tranche un par un — les bons entrent, et on
    // rapporte NOMMÉMENT ceux qui coincent.
    if (!/duplicate key|unique constraint/i.test(error.message || "")) {
      throw new Error(error.message || "Enregistrement refusé par la base");
    }
    for (const ligne of tranche) {
      const { data: un, error: e1 } = await supabase.from("catalogue_items").upsert(ligne).select().single();
      if (e1) ignores.push(ligne.nom);
      else if (un) sauves.push(versUi(un));
    }
  }
  sauves.ignores = ignores;
  return sauves;
}
...
export async function listerCatalogueRetires() {
  const data = await lireParPages(() =>
    supabase.from("catalogue_items").select("*").eq("actif", false).order("nom").order("id")
  );
  return (data || []).map(versUi);
}
