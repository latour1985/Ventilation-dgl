...
// 🔢 RÉGLER LE PROCHAIN NUMÉRO (2026-08-31, demande du propriétaire) —
// une entreprise qui arrive d'un autre système veut CONTINUER sa
// numérotation (reprendre à DEV-4880, pas à DEV-1). Le compteur stocke
// le DERNIER numéro émis : « prochain = N » s'écrit donc N-1. La RLS
// cloisonne par entreprise (clé composite du snippet 107) — chacun ne
// règle que les siens. On ne RECULE jamais : le garde est chez l'appelant
// (des numéros déjà émis existeraient en double).
export async function reglerProchainNumero(cle, prochain) {
  const valeur = Math.max(0, Math.floor(Number(prochain) || 0) - 1);
  const { error } = await supabase.from("compteurs").upsert({ cle, valeur });
  if (error) throw error;
}
