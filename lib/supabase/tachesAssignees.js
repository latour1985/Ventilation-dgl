...
export async function creerCourseTechnicien({ titre, adresse, note }, session) {
  ...
  // Échec annoncé ? On vérifie en base avant de le croire (voir
  // ligneExiste plus haut) — une réponse perdue n'est pas un échec.
  if (error && !(await ligneExiste(id))) throw error;
  return id;
}

// ⚠️ RÉPONSE PERDUE ≠ ÉCHEC (vécu sur iPhone/5G, 2026-08-20) : l'app
// annonçait « Impossible de créer la tâche » alors que la ligne ÉTAIT
// écrite en base — la requête s'était rendue, sa réponse non (réseau
// mobile qui bascule, onglet mis en veille par iOS). Le technicien
// recommençait donc et risquait de créer un doublon. Avant de déclarer
// un échec, on REGARDE si la ligne existe : c'est la base qui tranche,
// jamais la connexion.
async function ligneExiste(tacheId) {
  try {
