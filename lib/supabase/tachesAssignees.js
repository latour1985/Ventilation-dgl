...
import { estFerieCcq } from "../calendrierCcq";
...
// ============================================================
// ⚠️ PASSAGE OBLIGÉ PAR LE SERVEUR (diagnostic empirique 2026-08-20) :
// la RLS interdit à un TECHNICIEN d'écrire dans `taches_assignees` (le
// bureau planifie — et c'est très bien ainsi). Ces deux boutons
// échouaient donc depuis leur création pour tous les techniciens :
// « new row violates row-level security policy », zéro course jamais
// enregistrée. La route /api/taches/perso écrit à leur place avec la
// clé service, en n'acceptant QUE leur propre courriel (celui du jeton
// vérifié) et QUE les deux types sans client.
async function creerTachePerso(charge, session) {
  ...
  const n = new Date();
  // Jour et heure LOCAUX du téléphone (règle gelée : jamais toISOString
  // pour une date de calendrier).
  const jour = `${n.getFullYear()}-${String(n.getMonth() + 1).padStart(2, "0")}-${String(n.getDate()).padStart(2, "0")}`;
  const heure = `${String(n.getHours()).padStart(2, "0")}:${String(n.getMinutes()).padStart(2, "0")}`;
  const reponse = await fetch("/api/taches/perso", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${jeton}` },
    body: JSON.stringify({ ...charge, jour, heure }),
  });
  const resultat = await reponse.json().catch(() => ({}));
  if (!reponse.ok) throw new Error(resultat?.erreur || `Refus du serveur (code ${reponse.status}).`);
  return resultat?.id || null;
}

export async function creerCourseTechnicien({ titre, adresse, note }, session) {
  return creerTachePerso({ type: "course", titre, adresse: adresse || "", note: note || "" }, session);
}

// 🏭 TRAVAIL AU SHOP créé par le technicien (2026-08-19) — même recette
// que la course : pour lui, aujourd'hui, aucun client, rien à facturer.
// Il termine sa dernière tâche de la journée, arrive au bureau, pèse le
// bouton — ses heures continuent de compter (catégorie « divers »).
// `projetProposeId` (2026-08-31, facultatif) : le technicien PROPOSE le
// projet concerné — le bureau confirme avant que le lien compte.
export async function creerTravailShopTechnicien({ titre, note, projetProposeId }, session) {
  return creerTachePerso({ type: "shop", titre, note: note || "", projetProposeId: projetProposeId || null }, session);
}

// 🏗️ Les projets offerts au sélecteur du travail au shop (noms
// seulement — la route sert le strict minimum, la RLS reste fermée).
export async function listerProjetsPourTechnicien(session) {
  try {
    let jeton = session?.access_token;
    if (!jeton) {
      const { data } = await supabase.auth.getSession();
    ...
    if (!jeton) return [];
    const reponse = await fetch("/api/taches/perso", { headers: { Authorization: `Bearer ${jeton}` } });
...
// ✅/✗ TRAITEMENT PAR LE BUREAU d'une proposition de projet sur un
// travail au shop : confirme (projet_id posé) ou refuse (proposition
// effacée, rien d'autre ne bouge). Écrit la COLONNE et la fiche
// `donnees` d'un même geste — l'agenda Realtime suit tout seul.
export async function traiterPropositionProjetShop(tacheId, projetId) {
  // Une tâche shop appartient à UN technicien (id « shop-<horodatage> ») :
  // la retrouver par son seul tache_id est sûr.
  ...
  const { error: e2 } = await supabase
    ...
    .update({
      ...
      donnees: {
        ...d,
        ...(projetId ? { projetId } : {}),
        projetProposeId: null,
        projetProposeNom: null,
        projetProposePar: null,
      },
    })
    .eq("id", data.id);
  if (e2) throw e2;
...
// Respecte « sauter les fins de semaine » — et « sauter les jours
// fériés (CCQ) » (2026-08-31) — quand l'agenda l'a demandé. MÊME
// mécanique que calculerJoursCibles côté bureau : les deux doivent
// produire les mêmes journées, sinon le technicien verrait sa tâche
// un jour où le bureau ne l'a pas cédulée.
function joursDeLaTache(dateDebut, nbJours, sauterWeekend, sauterFeries) {
  const total = Math.max(1, Number(nbJours) || 1);
  const jours = [];
  let curseur = dateDebut;
  let garde = 0;
  while (jours.length < total && garde < 60) {
    const saute = (sauterWeekend && estFinDeSemaine(curseur)) || (sauterFeries && estFerieCcq(curseur));
    if (!saute) jours.push(curseur);
...
export async function listerTachesPourEmploye(courriel) {
  if (!courriel) return [];
  // 📚 Par pages (bilan 2026-09-03) : l'historique d'un technicien
  // dépasse 1 000 lignes en un an — le plafond Supabase coupait la fin.
  const data = await lireParPages(() =>
    supabase.from("taches_assignees").select("*").eq("employe_email", courriel.toLowerCase()).order("date_debut").order("id")
  );

  const lignes = [];
  (data || []).forEach((row) => {
    const nbJours = Math.max(1, Number(row.jours) || 1);
    const jours = joursDeLaTache(row.date_debut, nbJours, !!row.donnees?.sauterWeekend, !!row.donnees?.sauterFeries);
  ...
  return lignes.map(({ row, jour, index, nbJours }) => ({
    ...
    // 🛡️ Retour sous garantie (cochée par le bureau à la création) —
    // le courriel de fin de travaux OMET alors la demande d'avis Google.
    garantie: !!row.donnees?.garantie,
    // ✅ Étapes de la job (checklist du bureau, 2026-09-09) — cochées
    // sur le terrain, elles suivent le bon jusqu'au dossier client.
    etapes: Array.isArray(row.donnees?.etapes) ? row.donnees.etapes : [],
    ...
    // 🚪 App./bureau/casier — affiché en évidence sur la tâche (le lien
    // Google Maps, lui, reste sans l'unité : Maps s'y perd).
    adresseUnite: row.donnees?.adresseUnite || null,
    ...
    // 🔧 UNITÉS CONCERNÉES (2026-08-25) — cochées par le bureau depuis
    // le carnet d'équipements du client : le technicien sait LAQUELLE
    // travailler quand il y en a trois sur le toit, et sa section
    // « Unité vérifiée » arrive pré-remplie avec le vrai Nº de série.
    unites: Array.isArray(row.donnees?.unites) ? row.donnees.unites : [],
    ...
    // 🔒 Note INTERNE du bureau pour le technicien (2026-09-22) — affichée
    // sur son téléphone seulement, jamais sur un document client.
    noteBureau: row.donnees?.noteBureau || "",
    // 🚚 Tournée de ramassage (2026-09-21) : les bons à aller chercher.
    ramassages: Array.isArray(row.donnees?.ramassages) ? row.donnees.ramassages : [],
...
export async function listerToutesAssignations() {
  // 📚 Par pages (bilan 2026-09-03) — la table entière grossit chaque jour.
  return lireParPages(() => supabase.from("taches_assignees").select("*").order("date_debut").order("id"));
}

// Abonnement Realtime : rappelé à chaque changement d'assignation.
// ⚠️ NOM DE CANAL UNIQUE (2026-08-22) : l'agenda s'abonne DEUX fois —
// une pour la couleur « en cours » (immédiate), une pour recharger la
// grille. Deux canaux du même nom se marchent dessus et l'un des deux
// cesse de recevoir.
let compteurCanauxAssignees = 0;
export function sAbonnerTachesAssignees(onChangement) {
  compteurCanauxAssignees += 1;
  const canal = supabase
    .channel(`taches-assignees-${compteurCanauxAssignees}`)
