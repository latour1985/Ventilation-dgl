...
export async function creerAchatLibre({ numeroBc, fournisseurNom, description, montantHT, dateAchat, tacheId, tacheTitre, clientId, clientNom, montantAttribue, livraisonSouhaitee = null, ramassePar = null, depotA = null }, session) {
  const base = {
    ...
    // 📦 Date de livraison (snippet 147) — l'app relit aussi le texte.
    ...(livraisonSouhaitee ? { livraison_souhaitee: livraisonSouhaitee } : {}),
    // 🚚 Ramassage (snippet 153) : qui y va, où déposer le matériel.
    ...(ramassePar ? { ramasse_par: String(ramassePar).toLowerCase() } : {}),
    ...(depotA ? { depot_a: depotA } : {}),
  };
  ...
  // Filet snippet 147 pas passé : l'achat entre sans la colonne de date
  // (la date reste lisible dans le texte de la description).
  if (error && livraisonSouhaitee && /livraison_souhaitee/.test(error.message || "")) {
    const { livraison_souhaitee: _l, ...sansDate } = avecRattachement;
    ({ error } = await supabase.from("achats_libres").insert(sansDate));
  }
  // Filet snippet 153 pas passé : le bon entre sans « qui ramasse / où
  // déposer » (le texte du bon, lui, dit quand même « ramassage »).
  if (error && /ramasse_par|depot_a/.test(error.message || "")) {
    const { ramasse_par: _r, depot_a: _d, ...sansRamassage } = avecRattachement;
    ({ error } = await supabase.from("achats_libres").insert(sansRamassage));
  }
  if (error) throw error;
}

// 📦 Date de livraison lue dans le TEXTE d'un BC créé avant le snippet 147
// (« 📦 Livraison souhaitée : mardi 15 septembre 2026 ») → ISO.
...
export function livraisonDepuisTexte(description) {
  const m = String(description || "").match(/Livraison souhaitée\s*:\s*(?:[a-zéû]+\s+)?(\d{1,2})(?:er)?\s+([a-zéû]+)\s+(\d{4})/i);
  if (!m) return null;
...
export async function majAchatLibre(id, champs) {
  ...
  // ✉️ Trace d'envoi du BC au fournisseur (snippet 146, 2026-09-15).
  if (champs.bcEnvoyeLe !== undefined) maj.bc_envoye_le = champs.bcEnvoyeLe || null;
  if (champs.bcEnvoyeA !== undefined) maj.bc_envoye_a = Array.isArray(champs.bcEnvoyeA) ? champs.bcEnvoyeA : [];
  // 📦 Livraison prévue / reçue (snippet 147).
  if (champs.livraisonSouhaitee !== undefined) maj.livraison_souhaitee = champs.livraisonSouhaitee || null;
  if (champs.recuLe !== undefined) maj.recu_le = champs.recuLe || null;
  // ⚠️ Réception PARTIELLE (snippet 155, 2026-09-22).
  if (champs.manquant !== undefined) maj.manquant = champs.manquant || null;
  if (champs.restePromisLe !== undefined) maj.reste_promis_le = champs.restePromisLe || null;
  if (champs.partielLe !== undefined) maj.partiel_le = champs.partielLe || null;
  if (champs.reclameLe !== undefined) maj.reclame_le = champs.reclameLe || null;
  // 🚚 Ramassage (snippet 153).
  if (champs.ramassePar !== undefined) maj.ramasse_par = champs.ramassePar ? String(champs.ramassePar).toLowerCase() : null;
  if (champs.depotA !== undefined) maj.depot_a = champs.depotA || null;
  if (Object.keys(maj).length === 0) return;
  let { error } = await supabase.from("achats_libres").update(maj).eq("id", id);
  // Filet snippet 153 pas passé : la fiche s'enregistre sans les champs
  // de ramassage plutôt que d'échouer en bloc.
  if (error && /ramasse_par|depot_a/.test(error.message || "")) {
    delete maj.ramasse_par;
    delete maj.depot_a;
    if (Object.keys(maj).length === 0) return;
    ({ error } = await supabase.from("achats_libres").update(maj).eq("id", id));
  }
...
export async function listerAchatsLibres() {
  ...
  return (data || []).map((row) => ({
    ...
    // ✉️ Envoyé au fournisseur ? (snippet 146) — null = jamais par Fluxya.
    bcEnvoyeLe: row.bc_envoye_le || null,
    bcEnvoyeA: Array.isArray(row.bc_envoye_a) ? row.bc_envoye_a : [],
    // 📦 Livraison : la colonne (147), sinon la date écrite dans le texte.
    livraisonSouhaitee: row.livraison_souhaitee || livraisonDepuisTexte(row.description),
    recuLe: row.recu_le || null,
    // ⚠️ Réception partielle (snippet 155) — absents tant que le snippet n'est pas passé.
    manquant: row.manquant || "",
    restePromisLe: row.reste_promis_le || null,
    partielLe: row.partiel_le || null,
    reclameLe: row.reclame_le || null,
    // 🚚 Ramassage (snippet 153) — absents tant que le snippet n'est pas passé.
    ramassePar: row.ramasse_par || null,
    depotA: row.depot_a || "",
    ramassageNote: row.ramassage_note || "",
  }));
}
