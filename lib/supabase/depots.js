...
function versUi(row) {
  return {
    ...
    qboDocNumber: row.qbo_depot_doc_number || null,
...
export async function listerDepots() {
  // 📚 Par pages (bilan 2026-09-03) — les dépôts s'accumulent avec les appels.
  const data = await lireParPages(() => supabase.from("depots").select("*").order("tache_id"));
  const parTache = {};
  (data || []).forEach((r) => {
...
// système) : le client a payé son dépôt AVANT Fluxya — on enregistre le
// fait, avec le Nº de la facture QuickBooks existante en référence.
// AUCUNE facture n'est créée, AUCUN courriel ne part : la tâche est
// planifiable tout de suite, le fil comptable est gardé.
export async function creerDepotDejaPaye(tacheId, { montantHT, modePaiement, refFacture, parNom }) {
  const maintenant = new Date().toISOString();
  const { error } = await supabase.from("depots").upsert({
    tache_id: tacheId,
    statut: "paye_manuellement",
    montant_ht: Number(montantHT) || 0,
    date_limite: maintenant, // déjà réglé — aucune échéance à courir
    is_prospect: false,
    // Le Nº de facture voyage AVEC le mode : il s'affiche sur le badge
    // « 💰 DÉPÔT REÇU (…) » de la tâche sans rien changer d'autre.
    mode_paiement: refFacture ? `${modePaiement} — facture QB Nº ${refFacture}` : modePaiement,
    paye_le: maintenant,
    paye_par: parNom || "transfert d'un autre système",
    qbo_depot_doc_number: refFacture || null,
  });
  if (error) throw error;
}

// Numéro et id de la facture de dépôt QuickBooks — posés après sa
// création par la route serveur (ou remis à null après un VOID).
export async function majDepotFactureQbo(tacheId, { factureId, docNumber }) {
  const { error } = await supabase
    .from("depots")
    .update({ qbo_depot_invoice_id: factureId || null, qbo_depot_doc_number: docNumber || null })
    .eq("tache_id", tacheId);
  if (error) throw error;
}
