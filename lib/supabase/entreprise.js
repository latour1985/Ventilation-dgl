...
export const CONFIG_DEFAUT = {
  ...
  seuilMargeAlerte: 25,
  // 💼 Charges de l'employeur (RRQ, AE, RQAP, CNESST, vacances…) ajoutées
  // au salaire dans le COÛT d'un employé (snippet 153). 0 = pas encore réglé.
  chargesEmployeurPct: 0,
  ...
  // ⭐ Lien d'avis Google (profil d'entreprise Google → « Demander des
  // avis ») — collé une fois dans Paramètres, il s'ajoute au courriel
  // « Vos travaux sont terminés ». Vide = aucun bloc d'avis.
  lienAvisGoogle: "",
  // 🧮 SYSTÈME COMPTABLE de l'entreprise (chantier Sage, 2026-09-07) :
  // "quickbooks" (défaut — rien ne change pour les entreprises
  // existantes), "sage" (Sage Business Cloud) ou "aucun".
  systemeComptable: "quickbooks",
  ...
  calendrierCcq: false, // suivre le calendrier CCQ (construction seulement)
  ...
  logoDonnees: "",
  // 📧 Copie permanente des BONS DE COMMANDE (2026-09-03, demande du
  // propriétaire) : une adresse (ex. commande@...) mise en CC de CHAQUE
  // bon de commande envoyé — en plus de celui qui commande. Vide = pas
  // de copie permanente. Par entreprise, comme tout le reste.
  courrielCopieBc: "",
};
...
function versUi(row) {
  return {
    ...
    // 📅 Calendrier CCQ (snippet 118) — EN OPTION : décoché par défaut,
    // seules les compagnies de construction le cochent.
    calendrierCcq: row.calendrier_ccq === true,
    ...
    chargesEmployeurPct: row.charges_employeur_pct != null ? Number(row.charges_employeur_pct) : CONFIG_DEFAUT.chargesEmployeurPct,
    envoiAutoBonClient: row.envoi_auto_bon_client !== false,
    lienAvisGoogle: row.lien_avis_google || "",
    systemeComptable: ["sage", "aucun"].includes(row.systeme_comptable) ? row.systeme_comptable : "quickbooks",
    ...
    // 📦 Ne remonter que les dépenses portant un Nº de BC (snippet 116)
    // — pour les entreprises à fort volume de transactions hors chantier.
    achatsSeulementBc: row.achats_seulement_bc === true,
    ...
    courrielCopieBc: row.courriel_copie_bc || "",
  };
}
...
export async function sauvegarderEntreprise(c) {
  const ligne = {
    ...
    calendrier_ccq: c.calendrierCcq === true,
    ...
    charges_employeur_pct: c.chargesEmployeurPct ?? 0,
    envoi_auto_bon_client: c.envoiAutoBonClient !== false,
    lien_avis_google: String(c.lienAvisGoogle || "").trim() || null,
    systeme_comptable: ["sage", "aucun"].includes(c.systemeComptable) ? c.systemeComptable : "quickbooks",
    ...
    achats_seulement_bc: !!c.achatsSeulementBc,
    transport_quotidien_paye: c.transportQuotidienPaye !== false,
    logo_donnees: c.logoDonnees || null,
    courriel_copie_bc: c.courrielCopieBc || null,
  };
  // 🔐 UPDATE — jamais upsert (correctif 2026-09-06) : depuis les
  // cloisons du grand soir, INSÉRER dans `entreprises` est réservé à la
  // plateforme (une fiche naît dans la console, jamais ici). Or un
  // upsert est un INSERT … ON CONFLICT : Postgres exige la permission
  // d'INSERTION même quand la ligne existe — la sauvegarde des
  // Paramètres échouait donc pour TOUT admin sans sceau plateforme
  // (« Masquage affiché mais NON enregistré » chez Ventilation Miroir ;
  // invisible chez DGL car le compte du propriétaire porte le sceau).
  const { id: idFiche, ...champs } = ligne;
  const maj = (objet) => supabase.from("entreprises").update(objet).eq("id", idFiche);
  const { error } = await maj(champs);
  ...
  if (error && (error.code === "PGRST204" || /qb_lecture_depuis|logo_donnees|achats_seulement_bc|courriel_copie_bc|lien_avis_google|systeme_comptable|charges_employeur_pct/.test(error.message || ""))) {
    const { qb_lecture_depuis: _ignore1, logo_donnees: _ignore2, achats_seulement_bc: _ignore3, courriel_copie_bc: _ignore4, lien_avis_google: _ignore5, systeme_comptable: _ignore6, charges_employeur_pct: _ignore7, ...sansColonnesRecentes } = champs;
    const { error: e2 } = await maj(sansColonnesRecentes);
    if (e2) throw e2;
    return;
  }
  if (error) throw error;
...
// 📧 Écriture CIBLÉE de l'adresse de copie permanente des bons de
// commande — une seule colonne, jamais toute la fiche : la case
// « retenir » de la modale d'envoi ne doit pas réécrire des réglages
// possiblement périmés dans l'onglet ouvert depuis le matin.
export async function poserCopieBc(entrepriseId, courriel) {
  const { error } = await supabase
    .from("entreprises")
    .update({ courriel_copie_bc: String(courriel || "").trim() || null })
    .eq("id", entrepriseId || "dgl");
  if (error) throw error;
}
