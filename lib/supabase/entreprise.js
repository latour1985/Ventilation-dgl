...
export async function sauvegarderEntreprise(c) {
  ...
  // ⚠️ FILET COLONNE MANQUANTE : si le snippet SQL 81 n'a pas encore été
  // passé, la sauvegarde des Paramètres continue de fonctionner — la
  // date-plancher, elle, attendra le snippet.
  if (error && (error.code === "PGRST204" || /qb_lecture_depuis/.test(error.message || ""))) {
    const { qb_lecture_depuis: _ignore, ...sansPlancher } = ligne;
    const { error: e2 } = await supabase.from("entreprises").upsert(sansPlancher);
    if (e2) throw e2;
    return;
  }
  if (error) throw error;
