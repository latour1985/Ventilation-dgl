...
function versUi(row) {
  return {
    ...
    // 👁️ Consultations du client (snippet 123) — absentes tant que le
    // snippet n'est pas passé : la carte ne montre alors rien.
    ...
    // 🏠 Adresse des travaux (snippet 112) — texte figé sur le devis.
    adresseTravaux: row.adresse_travaux || null,
    // 🏷️ Titre du devis (snippet 149) — sert à l'identifier quand il n'y
    // a pas d'adresse (« Remplacement thermopompe », « Entretien annuel »).
    titre: row.titre || "",
    // 🔔 Relances envoyées (snippet 130) — [{ date, courriels }] ; vide
    // tant que le snippet n'est pas passé.
    relances: Array.isArray(row.relances) ? row.relances : [],
  };
}
...
// Tous les devis, par pages (revue 2026-09-22 — plafond de 500 : 39 devis
// aujourd'hui, mais les dossiers plus anciens auraient disparu en silence).
export async function listerDevis() {
  const data = await lireParPages(() =>
    supabase.from("devis_app").select("*").order("created_at", { ascending: false }).order("id", { ascending: true })
  );
...
export async function sauvegarderDevis(d) {
  ...
  const ligne = {
    ...
    adresse_travaux: d.adresseTravaux || null,
    titre: d.titre || null,
    relances: d.relances || [],
  };
  ...
  // Snippet 112 pas encore passé : le devis s'enregistre sans l'adresse.
  if (error && /adresse_travaux/.test(error.message || "")) {
    const { adresse_travaux: _absent3, ...sansAdresse } = ligne;
    const { error: e4 } = await supabase.from("devis_app").upsert(sansAdresse);
    if (e4) throw e4;
    return;
  }
  // Snippet 149 pas encore passé : le devis s'enregistre sans le titre.
  if (error && /titre/.test(error.message || "")) {
    const { titre: _absent5, ...sansTitre } = ligne;
    const { error: e6 } = await supabase.from("devis_app").upsert(sansTitre);
    if (e6) throw e6;
    return;
  }
  // Snippet 130 pas encore passé : le devis s'enregistre sans la trace
  // des relances (le courriel, lui, est déjà parti).
  if (error && /relances/.test(error.message || "")) {
    const { relances: _absent4, ...sansRelances } = ligne;
    const { error: e5 } = await supabase.from("devis_app").upsert(sansRelances);
    if (e5) throw e5;
    return;
  }
  if (error) throw error;
...
// 🛡️ BASCULE ATOMIQUE (snippet 141, audit 2026-09-09) : transfert du
// jeton public (le lien du client) + changement de version active dans
// UNE transaction Postgres. Avant, une coupure entre « libérer le
// jeton de l'ancienne » et « le poser sur la nouvelle » laissait le
// lien du client MORT — ou un dossier sans aucune version active.
// Retourne true si la fonction a fait le travail ; false si le snippet
// n'est pas encore passé (l'appelant retombe sur l'ancien chemin).
export async function basculerVersionDevis(numeroBase, nouveauId, ancienId) {
  const { error } = await supabase.rpc("basculer_version_devis", {
    p_numero_base: numeroBase,
    p_nouveau_id: nouveauId,
    p_ancien_id: ancienId || null,
  });
  if (!error) return true;
  if (error.code === "PGRST202" || /find the function|does not exist/i.test(String(error.message || ""))) return false;
  throw error;
}
