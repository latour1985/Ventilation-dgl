...
// 📚 TOUTES les lignes, par pages (revue 2026-09-22 : plafond de 500 pour
// toute l'entreprise depuis le premier dépôt — 217 lignes aujourd'hui, mais
// passé 500 les semaines de paie et les coûts de projets plus anciens
// auraient été faux en silence).
export async function listerTravauxEffectues() {
  const data = await lireParPages(() =>
    supabase.from("travaux_effectues").select("*").order("date_travail", { ascending: false }).order("id", { ascending: true })
  );
...
export async function listerTravauxPourEmploye(courriel) {
  ...
  const { data, error } = await supabase
    ...
    .limit(2000); // un employé : ~1 an et demi de pointages (revue 2026-09-22 — c'était 500)
  if (error) throw error;
  return (data || []).map(versUiTravail);
}
...
// Charles : une job planifiée mercredi, partie mardi → ses heures
// tombaient mercredi). Seule la DATE bouge — heures, taux figé, notes,
// photos restent. Les horodatages réels suivent du même nombre de jours
// (en heure locale : pas de dérive d'une heure au changement d'heure).
// Le bon de travail de la tâche, s'il porte la même date, suit aussi
// (c'est lui qui donne la date de la facture).
// ============================================================
export async function deplacerLigneHeures(travail, nouvelleDate) {
  const ancienne = String(travail?.date || "").slice(0, 10);
  if (!ancienne || !nouvelleDate || ancienne === nouvelleDate) return { bonDeplace: false, debutReel: travail?.debutReel || null, finReelle: travail?.finReelle || null };
  const decalage = Math.round((new Date(`${nouvelleDate}T12:00:00`) - new Date(`${ancienne}T12:00:00`)) / 86400000);
  const decaler = (ts) => {
    if (!ts) return null;
    const d = new Date(ts);
  ...
  const maj = { date_travail: nouvelleDate };
  if (travail.debutReel) maj.debut_reel = decaler(travail.debutReel);
  if (travail.finReelle) maj.fin_reelle = decaler(travail.finReelle);
  // 🔑 La clé (tache_id, employé) est UNIQUE.
  //  • Transport et dîner : la clé porte le JOUR RÉEL (« transport-debut-
  //    <jour> », « lunch-<jour> ») — elle suit la date, sinon le pointage
  //    du lendemain ÉCRASERAIT la ligne déplacée.
  //  • Tâche (« id » ou « id::<jour> ») : la clé est celle de la CARTE
  //    planifiée sur le téléphone — on n'y touche plus (revue 2026-09-22 :
  //    la réécrire cassait le lien carte ↔ heures : écrasement ou doublon).
  const ancienneCle = String(travail.tacheId || "");
  const cleDuJour = /^(transport-|lunch-)/.test(ancienneCle);
  if (cleDuJour && ancienneCle.includes(ancienne)) maj.tache_id = ancienneCle.split(ancienne).join(nouvelleDate);
  const { error } = await supabase.from("travaux_effectues").update(maj).eq("id", idBrut(travail.id));
  if (error) {
    if (error.code === "23505") throw new Error("Cet employé a déjà une ligne de cette même tâche à cette date — corrige plutôt les heures de la ligne existante.");
    throw error;
  }
  let bonDeplace = false;
  const tacheId = String(travail.tacheId || "").split("::")[0];
  if (tacheId && !tacheId.startsWith("lunch-") && !tacheId.startsWith("transport-")) {
    // SON bon seulement (revue 2026-09-22 : déplacer les heures de Charles
    // déplaçait aussi le bon de ses coéquipiers).
    const { data } = await supabase
      .from("bons_travail")
      .update({ date_travail: nouvelleDate })
      .eq("tache_id", tacheId)
      .eq("date_travail", ancienne)
      .eq("employe_email", String(travail.employeEmail || "").toLowerCase())
      .select("id");
    bonDeplace = (data || []).length > 0;
  }
  return { bonDeplace, tacheId: maj.tache_id || travail.tacheId, debutReel: maj.debut_reel ?? travail.debutReel ?? null, finReelle: maj.fin_reelle ?? travail.finReelle ?? null };
}
...
// 💵 `semainePayee` (revue 2026-09-22) : une journée bloquée est EXCLUE de
// la paie de sa semaine. Débloquée APRÈS que cette paie est faite, ses
// heures n'ont jamais été versées : on les marque « corrigées maintenant,
// payées 0 h » → elles partent en Report ± sur la semaine courante.
export async function debloquerJournee(email, date, { semainePayee = false } = {}) {
  if (semainePayee) {
    const { error: e0 } = await supabase
      .from("travaux_effectues")
      .update({ corrige_le: new Date().toISOString(), heures_avant_correction: 0 })
      .eq("employe_email", (email || "").toLowerCase())
