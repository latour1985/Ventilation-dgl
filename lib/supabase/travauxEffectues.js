...
export async function deplacerLigneHeures(travail, nouvelleDate) {
  ...
  // 🔑 La clé (tache_id, employé) est UNIQUE et porte souvent la date
  // (« transport-debut-<jour> », « lunch-<jour> », « id::<jour> ») : si on
  // laissait l'ancienne date dans la clé, un pointage ultérieur du même
  // jour ÉCRASERAIT la ligne déplacée. La clé suit donc la date.
  const ancienneCle = String(travail.tacheId || "");
  if (ancienneCle.includes(ancienne)) maj.tache_id = ancienneCle.split(ancienne).join(nouvelleDate);
  ...
  if (tacheId && !tacheId.startsWith("lunch-") && !tacheId.startsWith("transport-")) {
    const { data } = await supabase.from("bons_travail").update({ date_travail: nouvelleDate }).eq("tache_id", tacheId).eq("date_travail", ancienne).select("id");
