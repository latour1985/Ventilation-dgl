...
export async function listerEntreprisesPlateforme() {
  // `select *` (2026-09-14) : la liste nommée cassait dès qu'une colonne
  // récente (résiliation) n'était pas encore en base — l'étoile tolère
  // les snippets pas encore passés, le mapping fait le tri.
  ...
  return (data || []).map((r) => ({
    ...
    // ❌ Résiliation d'abonnement (snippet 132) — absente tant que le
    // snippet n'est pas passé.
    resilieLe: r.resilie_le || null,
    resilieRaison: r.resilie_raison || "",
    statutAvantResiliation: r.statut_avant_resiliation || null,
...
// 🏢 CRÉER UNE ENTREPRISE CLIENTE — fiche + invitation de SON admin
// principal, étiqueté à SA nouvelle bulle (route service : le verrou
// d'isolation et le niveau clé principale y sont re-vérifiés).
export async function creerEntreprisePlateforme(corps) {
  const { data } = await supabase.auth.getSession();
  const jeton = data?.session?.access_token;
  if (!jeton) return { erreur: "Session expirée." };
  try {
    const reponse = await fetch("/api/plateforme/entreprises", {
      method: "POST",
...
// et promesse de l'entente fondateurs. Depuis 2026-09-06 la lecture se
// fait par la ROUTE SERVICE /api/plateforme/export (la liste des
// tables vit là-bas) : le navigateur, cloisonné par RLS, ne voyait
// plus rien d'une entreprise étrangère — l'export sortait VIDE sans
// le dire.
// ------------------------------------------------------------
...
export async function exporterEntreprise(entrepriseId) {
  ...
  if (!jeton) return { erreur: "Session expirée." };
  try {
    const reponse = await fetch("/api/plateforme/export", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${jeton}` },
