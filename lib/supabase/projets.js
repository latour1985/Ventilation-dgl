...
function versUi(row) {
  return {
    ...
    // 🔢 Nº de suivi du CLIENT (2026-09-06) — part sur chaque facture du projet.
    numeroSuiviClient: row.numero_suivi_client || "",
...
export async function listerProjets() {
  // 📚 Par pages (bilan 2026-09-03) — les chantiers s'accumulent avec les années.
  const data = await lireParPages(() => supabase.from("projets_app").select("*").order("created_at", { ascending: false }).order("id"));
...
export async function sauvegarderProjet(p) {
  const ligne = {
    ...
    numero_suivi_client: p.numeroSuiviClient || null,
  ...
  if (error && (error.code === "PGRST204" || /reprise|numero_suivi_client/.test(error.message || ""))) {
    const { reprise: _sansReprise, numero_suivi_client: _sansSuivi, ...sansColonneRecente } = ligne;
