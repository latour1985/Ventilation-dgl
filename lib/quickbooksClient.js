...
// SOLDES RÉELS d'un lot de factures (payée ? en retard ?) — lus du
// registre QuickBooks : { factures: { id: { solde, payee, enRetard… } } }.
export async function lireSoldesQbo(ids) {
  try {
    const jeton = await jetonSession();
    if (!jeton) return { erreur: "Session expirée — reconnecte-toi." };
    const reponse = await fetch("/api/quickbooks/paiements", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${jeton}` },
      body: JSON.stringify({ action: "solde", ids }),
    });
    return await reponse.json();
  } catch {
    return { erreur: "Réseau indisponible — réessaie." };
...
// ⏱️ TEMPS DE PAIEMENT MOYEN PAR CLIENT — paiements des 12 derniers
// mois rattachés à leurs factures soldées, moyenne et pire cas par
// client : { clients: [{ nom, nb, moyenneJours, pireJours }], global }.
export async function lireDelaisPaiementQbo() {
  try {
    const jeton = await jetonSession();
    if (!jeton) return { erreur: "Session expirée — reconnecte-toi." };
    const reponse = await fetch("/api/quickbooks/paiements", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${jeton}` },
      body: JSON.stringify({ action: "delais" }),
    });
    return await reponse.json();
  } catch {
    return { erreur: "Réseau indisponible — réessaie." };
...
// 💳 NOTES DE CRÉDIT (CreditMemo) du registre QuickBooks : { credits: [...] }
// — pour que les remboursements aux clients baissent le facturé dans
// l'analyse de rentabilité.
export async function lireCreditsQbo() {
  try {
    const jeton = await jetonSession();
    if (!jeton) return { erreur: "Session expirée — reconnecte-toi." };
    const reponse = await fetch("/api/quickbooks/paiements", {
...
// 💳 FRAIS DE TRAITEMENT (carte 2,9 %+0,25 $ / débit 1 %) — lus des
// paiements réels de QuickBooks (2026-09-17), pour les retrancher de la
// marge dans l'analyse de rentabilité.
export async function lireFraisPaiementQbo() {
  try {
    const jeton = await jetonSession();
    if (!jeton) return { erreur: "Session expirée — reconnecte-toi." };
    const reponse = await fetch("/api/quickbooks/paiements", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${jeton}` },
      body: JSON.stringify({ action: "frais-paiement" }),
    });
    return await reponse.json();
  } catch {
    return { erreur: "Réseau indisponible — réessaie." };
...
// COMPTES À RECEVOIR — toutes les factures au solde ouvert (même
// celles faites directement dans QuickBooks) : { ouvertes: [...] }.
export async function lireComptesARecevoirQbo() {
  try {
    const jeton = await jetonSession();
    if (!jeton) return { erreur: "Session expirée — reconnecte-toi." };
    const reponse = await fetch("/api/quickbooks/paiements", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${jeton}` },
      body: JSON.stringify({ action: "ouvertes" }),
    });
    return await reponse.json();
  } catch {
    return { erreur: "Réseau indisponible — réessaie." };
