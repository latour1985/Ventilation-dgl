...
export async function statutsEnvoiFactures(acces, ids) {
  ...
  const reponse = await requeteQbo(acces, `select Id, EmailStatus, DeliveryInfo from Invoice where Id in (${liste})`);
  const map = {};
...
export async function clientQboPour(acces, admin, { clientId, clientNom }) {
  ...
  if (!id) {
    const cree = await ecrireQbo(acces, "customer", { DisplayName: String(clientNom).slice(0, 100) });
