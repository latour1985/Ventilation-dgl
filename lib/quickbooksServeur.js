...
// ============================================================
const NOMS_CATEGORIES_QBO = { appel: "Appel de service", heures: "Heures" };
export async function articleQboParNom(acces, nom) {
  try {
    const lu = await requeteQbo(acces, `select Id, Name from Item where Name = '${echapperQbo(nom)}' maxresults 1`);
    if (lu?.Item?.[0]?.Id) return { id: lu.Item[0].Id, cree: false };
...
export async function resolveurArticlesQbo(acces, lignes, itemDefaut) {
  ...
  for (const c of categories) {
    ...
    if (r.cree) crees.push(NOMS_CATEGORIES_QBO[c]);
  }
  const pourLigne = (l) => {
