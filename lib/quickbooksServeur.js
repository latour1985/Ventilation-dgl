...
// app_metadata (scellée serveur). Plus de repli « dgl » (revue 2026-09-22) :
// un compte sans entreprise (opérateur de plateforme, invitation mal
// étiquetée) était traité comme un compte DGL par les routes à clé
// service, alors que la RLS ne lui montrait rien. Sans entreprise : un
// identifiant qui ne correspond à AUCUNE ligne — la route ne voit rien.
// (Vérifié : les 11 comptes existants portent tous leur entreprise.)
export function entrepriseDuCompte(utilisateur) {
  return utilisateur?.app_metadata?.entreprise_id || "__aucune__";
}
...
// 🔒 LE RÔLE SERVEUR (RLS phase 3, 2026-09-04) — lu de la TABLE des
// permissions, jamais de user_metadata : l'utilisateur peut modifier
// ses propres métadonnées via l'API Auth, un technicien pouvait donc
// se déguiser en admin auprès des routes. Sans ligne de permissions :
// Technicien, le rôle le moins privilégié (même règle que fn_mon_role
// du snippet 128). Toute erreur de lecture = Technicien aussi — on ne
// donne jamais un accès de bureau sur un doute.
export async function roleServeur(utilisateur) {
  try {
...
// 🔐 `entrepriseId` OBLIGATOIRE pour toucher une fiche (revue 2026-09-22) :
// la clé service voit toutes les entreprises — sans ce filtre, un
// administrateur d'une autre compagnie pouvait lire et relier une fiche
// de DGL en envoyant son identifiant. Sans entreprise : on ne lit ni
// n'écrit aucune fiche (le client QuickBooks est trouvé par son nom).
export async function clientQboPour(acces, admin, { clientId, clientNom, entrepriseId }) {
  let fiche = null;
  if (clientId && !entrepriseId) clientId = null;
  if (clientId) {
    const { data } = await admin
      .from("clients_app")
      ...
      .eq("entreprise_id", entrepriseId)
      .maybeSingle();
    if (!data) clientId = null; // fiche d'une autre entreprise (ou inexistante) : on n'y touche pas
  ...
  if (id && clientId) {
    await admin.from("clients_app").update({ quickbooks_customer_id: id, sync_qb: "synchronise" }).eq("id", clientId).eq("entreprise_id", entrepriseId);
  }
  return id;
}
...
export async function mettreAJourClientQbo(acces, admin, clientId, entrepriseId) {
  if (!entrepriseId) return false; // 🔐 revue 2026-09-22 — jamais une fiche hors de l'entreprise
...
// ============================================================
// 🏷️ LE BON PRODUIT/SERVICE PAR LIGNE (2026-09-18, demande du
// propriétaire : « toutes mes ventes tombent dans Services — je ne sais
// plus ce que je vends »). Chaque ligne peut porter :
//   • `itemId`    : l'article QuickBooks du produit (catalogue lié, ou
//                   article repris de la ligne du devis QuickBooks) ;
//   • `categorie` : "appel" → article « Appel de service »,
//                   "heures" → article « Heures » (main-d'œuvre T&M).
// Sans l'un ni l'autre : l'article général « Services », comme avant.
// Un article nommé absent du fichier est CRÉÉ (type Service) avec le
// même compte de revenus qu'un article de service existant — une seule
// fois, et l'appelant le dit au journal. Échec de création = repli sur
// « Services » : une facture ne bloque JAMAIS pour une catégorie.
// ============================================================
// ⚠️ VÉCU 2026-09-22 (facture de dépôt Giacomo Valzania refusée : « ID de
// référence non valide ») : le fichier de DGL a une CATÉGORIE nommée
// « Appel de service » (un dossier, Type = Category) — pas un article.
// La recherche par nom la prenait pour un article : QuickBooks refusait la
// facture. Désormais : seuls les VRAIS articles (Service / Inventory /
// NonInventory) comptent, et on essaie plusieurs noms dans l'ordre —
// ceux que DGL a déjà (« Appel de service taux horaire », « Dépôt ») avant
// d'en créer un.
const NOMS_CATEGORIES_QBO = {
  appel: ["Appel de service taux horaire", "Appel de service"],
  heures: ["Heures"],
  depot: ["Dépôt", "Depot", "Dépôt — appel de service"],
};
const TYPES_ARTICLE = ["Service", "Inventory", "NonInventory"];
export async function articleQboParNom(acces, noms) {
  const liste = Array.isArray(noms) ? noms : [noms];
  try {
    for (const nom of liste) {
      const lu = await requeteQbo(acces, `select Id, Name, Type, Active from Item where Name = '${echapperQbo(nom)}' maxresults 3`);
      const vrai = (lu?.Item || []).find((i) => TYPES_ARTICLE.includes(i.Type) && i.Active !== false);
      if (vrai?.Id) return { id: vrai.Id, cree: false };
    }
    const nom = liste[0];
    // Le compte de revenus se copie d'un article de service existant.
    const modele = await requeteQbo(acces, `select * from Item where Type = 'Service' maxresults 25`);
    const source =
      (modele?.Item || []).find((i) => i.Name === "Heures" && i.IncomeAccountRef) ||
      (modele?.Item || []).find((i) => i.Name === "Services" && i.IncomeAccountRef) ||
      (modele?.Item || []).find((i) => i.IncomeAccountRef);
    if (!source?.IncomeAccountRef) return { id: null, cree: false };
    const cree = await ecrireQbo(acces, "item", { Name: nom, Type: "Service", IncomeAccountRef: source.IncomeAccountRef });
    const id = cree?.Item?.Id || null;
    return { id, cree: !!id };
  } catch {
    return { id: null, cree: false };
  }
}
// Résout les articles d'un lot de lignes. Retourne :
//   pourLigne(l) → l'Id d'article à poser (jamais null si `itemDefaut`)
//   crees        → noms des articles créés à l'occasion (pour le journal)
export async function resolveurArticlesQbo(acces, lignes, itemDefaut) {
  const categories = [...new Set((lignes || []).map((l) => l?.categorie).filter((c) => NOMS_CATEGORIES_QBO[c]))];
  // Un itemId direct doit aussi être un VRAI article actif — sinon « Services ».
  const idsDirects = [...new Set((lignes || []).map((l) => String(l?.itemId || "").trim()).filter((x) => /^\d+$/.test(x)))];
  const idsValides = new Set();
  if (idsDirects.length > 0) {
    try {
      const lu = await requeteQbo(acces, `select Id, Type, Active from Item where Id in (${idsDirects.map((x) => `'${x}'`).join(",")}) maxresults 1000`);
      (lu?.Item || []).forEach((i) => { if (TYPES_ARTICLE.includes(i.Type) && i.Active !== false) idsValides.add(String(i.Id)); });
    } catch {
      // vérification impossible — on laissera QuickBooks trancher (repli générique en cas de refus)
      idsDirects.forEach((x) => idsValides.add(x));
    }
  }
  const parCategorie = {};
  const crees = [];
  for (const c of categories) {
    const r = await articleQboParNom(acces, NOMS_CATEGORIES_QBO[c]);
    if (r.id) parCategorie[c] = r.id;
    if (r.cree) crees.push(NOMS_CATEGORIES_QBO[c][0]);
  }
  const pourLigne = (l) => {
    const direct = String(l?.itemId || "").trim();
    if (idsValides.has(direct)) return direct;
    return parCategorie[l?.categorie] || itemDefaut;
  };
  return { pourLigne, crees };
}
